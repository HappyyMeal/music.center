	...
	<c:choose>
		<c:when test="${person == null}">
			<div class="slide story" id="slide-1" data-slide="1"
				...
				<form class="form-inline" role="form"
					action="/mc-client/artist/person/update" method="post">
		...
		<c:otherwise>
			<div class="slide story" id="slide-1" data-slide="1"
				style="height: 100%;">
				<form class="form-inline" role="form"
					action="/mc-client/artist/person/update/${person.artistId}"
					method="post">
					<label class="sr-only" for="firstName">Имя:</label>
					<div class="form-group">
						<input type="text" class="form-control" id="firstName"
							name="firstName" value="${person.firstName}" required>
					</div>
					<label class="sr-only" for="lastName">Фамилия:</label>
					<div class="form-group">
						<input type="text" class="form-control" id="lastName"
					...
					<div class="form-group">
						<input type='date' class="form-control" name="birthday"
							id="birthday" value="${strDate}" required />
					</div>
					<label class="sr-only" for="artistSiteUrl">Персональный
						сайт:</label>
					<div class="form-group">
						<input type='text' class="form-control" name="artistSiteUrl"
							id="artistSiteUrl" value="${person.artist.artistSiteUrl}"
							required />
					</div>
					...
					<div class="form-group">
						<select class="form-control" id="genre" name="genre">
							<c:forEach var="genre" items="${genres}">
								<c:choose>
									<c:when test="${person.artist.genre.genreId ==  genre.genreId}">
										<option value="${genre.genreId}" selected="selected">${genre.genreName}</option>
									</c:when>
									<c:otherwise>
					...
					<div class="form-group">
						<select class="form-control" id="sex" name="sex">
							<c:forEach var="sex" items="${sexs}">
								<c:choose>
									<c:when test="${person.sex.sexId eq sex.sexId}">
										<option value="${sex.sexId}" selected="selected">${sex.sexName}</option>
									</c:when>
									<c:otherwise>
										<option value="${sex.sexId}">${sex.sexName}</option>
									</c:otherwise>
					...
					<div class="form-group">
						<select class="form-control" id="country" name="country">
							<c:forEach var="country" items="${countries}">
								<c:choose>
									<c:when
										test="${person.artist.country.countryId eq country.countryId}">
